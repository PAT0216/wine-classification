...
corss_valid_df = pd.read_csv("../results/tables/cross_val_results.csv")
rbf_score = float(cross_valid_df.iloc[3, 1].split()[0])
Markdown(corss_valid_df.to_markdown(index=False))
```

Among all those evaluated models, the **RBF-SVM** achieved the highest validation performance and showed the smallest variability across folds, with mean CV score of `{python} rbf_score`, indicating a strong balance between accuracy and stability.
...
dummy_accuracy = test_metrics_df.loc[test_metrics_df["model"]=="dummy", "accuracy"].iloc[0]
dummy_accuracy_rounded = round(dummy_accuracy, 2)
mid_models = ["Decision Tree", "KNN", "Logistic Regression"]
mid_acc_values = test_metrics_df.loc[test_metrics_df["model"].isin(mid_models), "accuracy"].values
mid_min_accuracy = round(min(mid_acc_values), 2)
rbf_accuracy = float(test_metrics_df.loc[test_metrics_df["model"] == "RBF SVM", "accuracy"])
rbf_recall = float(test_metrics_df.loc[test_metrics_df["model"] == "RBF SVM", "recall"])
rbf_precision = float(test_metrics_df.loc[test_metrics_df["model"] == "RBF SVM", "precision"])
rbf_f1 = float(test_metrics_df.loc[test_metrics_df["model"] == "RBF SVM", "f1"])
