...
Overall, from @tbl-cv-df, most classifiers achieved strong predictive performance, except for the `DummyClassifier`, which is expected, since it is a baseline that does not use feature information.
...
cross_valid_df = pd.read_csv("../results/tables/cross_val_results.csv")
cross_valid_df = cross_valid_df.rename(columns={"Unnamed: 0": "model"})
rbf_test_cv = float(cross_valid_df.iloc[3, 3].split()[0])
rbf_train_cv = float(cross_valid_df.iloc[3, 4].split()[0])
Markdown(cross_valid_df.to_markdown(index=False))
```

Among all those evaluated models, the **RBF-SVM** achieved the highest validation performance and showed the smallest variability across folds, with mean CV train score of `{python} rbf_train_cv`, and test score of `{python} rbf_test_cv`, indicating a strong balance between accuracy and stability.
...
To further evaluate model behavior, we examined the confusion matrices for each classifier. The matrix provided a detailed view of how well each model distinguishes between red wine (label = 0) and white wine (label = 1).
Overall, we especially focus on the confusion matrix for RBF SVM model, and all models—except the DummyClassifier—achieve very strong separation between the two classes:

- **Decision Tree, KNN, RBF SVM, and Logistic Regression** all show very low wrong counts for both classes.
- **RBF SVM** performs especially well, from @fig-matrix we can see that it produced near-perfect predictions with only a small number of false positives and false negatives.

![Confusion matrix for the RBF SVM model.](../results/figures/confusion_matrix.png){#fig-matrix width=50%}
...
We next evaluated each model on the test set using **accuracy, recall, precision, and F1 score**. These metrics allow us to assess the generalization performance of each classifier beyond the training data and the result is shown in @tbl-test-metrics below.

```{python}
#| label: tbl-test-metrics
#| tbl-cap: Performance metrics (Accuracy, Recall, Precision, F1) for the best performing model (RBF SVM) on the test set.

test_metrics_df = pd.read_csv("../results/tables/test_metrics.csv")
test_metrics_df = test_metrics_df.rename(columns={"Unnamed: 0": "model"})
dummy_accuracy = test_metrics_df.iloc[0, test_metrics_df.columns.get_loc("accuracy")]
dummy_accuracy_rounded = float(round(dummy_accuracy, 4))
mid_indices = [1, 2, 4]
mid_acc_values = test_metrics_df.iloc[mid_indices, test_metrics_df.columns.get_loc("accuracy")].values
mid_min_accuracy = round(float(min(mid_acc_values)), 4)
rbf_accuracy = float(round(test_metrics_df.iloc[3, test_metrics_df.columns.get_loc("accuracy")], 4))
rbf_recall = float(round(test_metrics_df.iloc[3, test_metrics_df.columns.get_loc("recall")], 4))
rbf_precision = float(round(test_metrics_df.iloc[3, test_metrics_df.columns.get_loc("precision")], 4))
rbf_f1 = float(round(test_metrics_df.iloc[3, test_metrics_df.columns.get_loc("f1")], 4))
Markdown(test_metrics_df.to_markdown(index=False))
```

The results show a clear trend:

- The **DummyClassifier** performs the worst, with accuracy around `{python} dummy_accuracy_rounded` and substantially lower precision and F1.
- **Decision Tree, KNN, and Logistic Regression** all perform well, achieving accuracy scores above `{python} mid_min_accuracy` and balanced precision/recall values.
...
We found that all of the models that we tested preformed much better than our Dummy Classifier. Subsequently, comparing our models individually, each one performed well. However, the model that had the highest cross-validation score was RBF SVM at `{python} rbf_test_cv`. Since all models scored high (>0.99), we included them in calculating accuracy, recall, precision and f1 on our test data. Here again, we see RBF SVM performing the best with the highest scores in each one of these metrics (`{python} rbf_accuracy`, `{python} rbf_recall`, `{python} rbf_precision`, and `{python} rbf_f1` respectively).
